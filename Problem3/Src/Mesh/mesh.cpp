...
void IceMesh::AssignCoords()
{
    if (mesh_params.GetCoordsType() == CoordsType::Cartesian3D)
    {
            ...
            ++nodeit)
        {
            ...
            Spherical_Coords<double> model_coords(model_lon, model_lat);
            Spherical_Coords<double> geo_coords = Rotate_Spherical<double>(model_coords, rotation.Get_FORWARD());

            nodeit->RealArray(node_coords[NodeCoordsNotation::geo])[0] = geo_coords.Get_x()*(M_PI/180.0);
            nodeit->RealArray(node_coords[NodeCoordsNotation::geo])[1] = geo_coords.Get_y()*(M_PI/180.0);
    ...
    {
        INMOST_ICE_ERR("SPherical coordinates are not implemented yet");
    }
};
...
void IceMesh::AssembleLocalBasisData(bool display)
{
            ...
            ++trianit) 
    {
        ...
        if (mesh_params.GetCoordsType() == CoordsType::SphericalRadians)
        {
        ...
        {
            INMOST_ICE_ERR("currently only geo and Cartesian2D coords available");
        }
