...
c Local variables
      ...
      Real*8  PSI(2, 2)
      Real*8  U(4, 18, MaxPnt2DGauss),    V(4, 18, MaxPnt2DGauss)
      Real*8  Diff(4, 4, MaxPnt2DGauss), DU(4, 18, MaxPnt2DGauss)
...
c ... next two FEMs
      Else If(FEMtype.EQ.FEM_P2 .OR. FEMtype.EQ.FEM_P2vector) Then
         nfa = 6
         dim = 2
         If(FEMtype.EQ.FEM_P2vector)  Call clearU2D(iGauss, 18, 4, U)
...
c ... next two FEMs
      Else If(FEMtype.EQ.FEM_P2 .OR. FEMtype.EQ.FEM_P2vector) Then
         nfa = 6
         dim = 1
         If(FEMtype.EQ.FEM_P2vector) Call clearU2D(iGauss, 18, 3, U)
