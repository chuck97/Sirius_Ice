...
C  construction may use arrays DDATA, IDATA if neccessary. 
C
C
C *** SUBROUTINE fem3Dext ***
C  
C     SUBROUTINE fem3Dext(XY1, XY2, XY3, XY4,
C    &                    lbEloc, lbFloc, lbRloc, lbPloc, DDATA, IDATA, iSYS,
C    &                    LDA, A, F, nRow, nCol,
C    &                    templateR, templateC)
C
C     Parameters:
C              XY1(3) - Real*8 Cartesian coordinates of the first
C                         vertex of the tetrahedron
C              XY2(3) - second vertex of the tetrahedron
C              XY3(3) - third vertex of the tetrahedron
C              XY4(3) - fourth vertex of the tetrahedron
C
...
C                If your FEM program does not use edges or faces, the 
C                corresponding mesh maps are not created in this routine 
C                and the related entries in iSYS contain random numbers.
C
...
C                      A > 0 - output goes to file aniFEM.log (not suppoted yet)
C                      A = 0 - screen output is used
C ======================================================================
C *** Note:
C       Input parameters:  nP, nE, XYP, IPE, lbE,
C                          fem3Dext, DDATA, IDATA, assembleStatus,
C                          MaxIA, MaxA, MaxWi
C
...
c ... default values for flags
      ...
      If(flagCSR .AND. iPrint.GT.0) Then
...
c ... get template array
     ...
     &              lbE(1), lbFloc, lbRloc, lbPloc, DDATA, IDATA, iSYS,
...
c  ...   save edge labels
     ...
     &        lbE(n), lbFloc, lbRloc, lbPloc, DDATA, IDATA, iSYS,
     &        MaxSize, Aloc, Floc, ir, ic, 
     &        templateR, templateC)
...
c ... print itatistics
      ...
      If(iPrint.GT.0) Then
         Write(*,'(5X,A,I7,A,I7,A,I9,A)') 'matrix size:', nRow,
     &      ' x', nCol, '  (', non_zero, ' non-zero entries)'
