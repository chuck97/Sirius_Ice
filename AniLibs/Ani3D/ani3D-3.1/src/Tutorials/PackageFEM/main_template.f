...
c ... local variables
      Integer   i,n, iv1,iv2,iv3, ibc, iSYS(MAXiSYS)
      Real*8    x,y,z, eBC(3)
...
c ... Define a simple mesh for the prism consisting of 3 tets
      ...
      bnd(2,2) = 5
      bnd(3,2) = 6
      labelF(2) = 2   ! Neumann   face (z=1) - label 2
      bnd(1,3) = 1
      bnd(2,3) = 2
      bnd(3,3) = 4
...
c ... mark the Dirichlet points via labelP
      Do i = 1, nv 
         labelP(i) = 0
      End do
c     on intersection points of faces with different labels set label 6 (except labelF=2)
      Do n = 1, nb
         If(labelF(n).NE.2) Then
            Do i = 1, 3
               iv1 = bnd(i,n)
...
c ... impose boundary conditions (assume nRow = nCol) at mid-points of edges
     ...
     &          (lbP(i).EQ.lbP(j).OR.lbP(i).EQ.6.OR.lbP(j).EQ.6) ) Then
               x = (XYP(1, i) + XYP(1, j)) / 2
               y = (XYP(2, i) + XYP(2, j)) / 2
               z = (XYP(3, i) + XYP(3, j)) / 2
...
c  ...  Dbc may change iSYS
               lbc = min( lbP(i), lbP(j) )
               If (lbc.eq.6.and.z.le.0d0) lbc = 1 ! reset label for edge on inhomogeneous Dirichlet face
