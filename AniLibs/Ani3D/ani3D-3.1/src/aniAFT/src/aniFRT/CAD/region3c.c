...
void initAFSM_ (
	...
	int nSub;
	int nVert, nnVert = 0;
	double x, y, z, t, p[3], uv[2], pmin[3], pmax[3];
...
//			printf("\tline %d [%c]\n", k, (bInverse==1)?'*':((bInverse==0)?' ':'?'));
	...
	for (iLine=0; iLine<nLine; iLine++) {
		...
		nVert = 0;
		myReallocv(nVert+1, &nnVert, (nnVert > 1024)?1024:16, 2, &boundVert, sizeof(int), &boundT, sizeof(double));
		boundT[nVert] = tBegin;
		boundVert[nVert++] = vVert[vBegin];
		t = tBegin;
		for (j=0; ; j++) {
			if (nextUinEdge(pm, edge, t, &t)) break;

			pm->vert[pm->nPoint].u = t;
			CGM_GetEdgeCoordsFromU(edge, t, p);
			x = p[0];  y = p[1];  z = p[2];
			myReallocv(nVert+1, &nnVert, (nnVert > 1024)?1024:16, 2, &boundVert, sizeof(int), &boundT, sizeof(double));
			boundT[nVert] = t;
...
//				printf("Vertex %d: x=%lf, y=%lf, z=%lf, \tu=%lf, v=%lf\n", iVert,  pm->vert[iVert].x,  pm->vert[iVert].y,  pm->vert[iVert].z,  pm->vert[iVert].u, pm->vert[iVert].v);
			...
			bInverse = surface[iSurf].inverse[iLine] ^ surface[iSurf].iNorm;
		...
		for (i=0; i<pm->nTria; i++) {
			...
			if (bCut)
				addFace(pm, pm->tria[3*i+0], pm->tria[3*i+2], pm->tria[3*i+1], 1, iLabel);
		}
	}/*for(iSurf<nSurface)*/

	free(boundVert),  free(boundT),  nnVert = 0,  boundVert = NULL,  boundT = NULL;
	free(vVert);
