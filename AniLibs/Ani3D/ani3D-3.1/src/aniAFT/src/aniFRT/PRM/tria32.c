...
//		pm->sv[0].z = (p1*e1.z+p2*e2.z+p*norm.z)/len;
		surfNormal(pm, pm->iSurf, pm->iNorm,
			...
			(p1*pm->vert[v1].v+p2*pm->vert[v2].v+p*pm->vert[v].v)/len, &pm->sv);
		x += pm->sv.x*3.0/len,  y += pm->sv.y*3.0/len,  z += pm->sv.z*3.0/len;
		pm->nPoint++;
		addPoint(pm, x,y,z);
		pm->nPoint--;
		pm->nPoint--;
		...
		bln++;
		pm->ssect = pm->tvicinityFace[iFace];
...
void addBadVert(surface_mesh *pm, int v) {
    myReallocv(pm->nBadVert+1, &pm->maxBadVert, 16, 1, (void**)&pm->badVert, sizeof(int));
...
void addSphereVert(surface_mesh *pm, int v) {
    myReallocv(pm->nSphereVert+1, &pm->maxSphereVert, 16, 1, (void**)&pm->sphereVert, sizeof(int));
...
static int newPoint(surface_mesh *pm, PStrucEdge3 face) {
    ...
    if (pm->cf <= 0.0)  size = sizeFace(pm, 0.5*(x1+x2) - dist*0.57735027*((y2-y1)*surfNorm.z - (z2-z1)*surfNorm.y), 0.5*(y1+y2) - dist*0.57735027*((z2-z1)*surfNorm.x - (x2-x1)*surfNorm.z), 0.5*(z1+z2) - dist*0.57735027*((x2-x1)*surfNorm.y - (y2-y1)*surfNorm.x));
...
//    z = (pm->vert[v1].z+pm->vert[v2].z+pm->vert[pm->nPoint].z)/3.;
    ...
    if (bound < 0) {
	if (size < 1.6*dist)  size = 1.6*dist; /* workaround for sphere pole */
    }
...
*/
	...
	if ( (v[0] != -1) && (min < 0.6*size) )  pm->sworkVert = v[0];
...
static int addTria(surface_mesh *pm, int v1, int v2, int v3) {
    myReallocv(pm->nTria+1, &pm->maxTria, 1024, 1, (void**)&pm->tria, sizeof(int)*3);
    pm->tria[3*pm->nTria+0] = v1;
    pm->tria[3*pm->nTria+1] = v2;
    pm->tria[3*pm->nTria+2] = v3;
    return pm->nTria++;
} /*addTria*/
...
#ifdef SHOWPROGRESS
	printf("\r nP = %5d  nT = %5d  nE = %5d  FAIL\n", pm->nPoint, pm->nTria, pm->nEdge);
#endif
	if (0)  dump(pm);
	while (pm->nEdge)  remFace32(pm, pm->edge[0]);
