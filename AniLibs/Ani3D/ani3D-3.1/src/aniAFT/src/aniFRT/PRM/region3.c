...
static void rePlane(surface_mesh *pm, int vv) {
    double x, y, z, u, v;
...
static int nextTinEdge(surface_mesh *pm, int iSurf, int iV_U, double t, double *tout) {
    int i=0;
	...
	if (i > 100) {
	    if (fabs(s1-s) >= 10.*er)
		errorExit3(3, "i>100 in  nextTinEdge");
	    break;
	}
    }
    *tout = t;
...
void initAFS_ (
	...
	int *boundVert = NULL, *vVert;
	double *boundT = NULL, *boundU = NULL, *boundV = NULL;
	StrucLine3 *line;
...
//	printf("S0: %lf\n", S0);
	...
	for (iLine=0; iLine<nLine; iLine++) {
		...
		nVert = 0;
		bounLine(pm, iV_U, tBegin, &u, &v);
		myReallocv(nVert+1, &nnVert, (nnVert > 1024)?1024:16, 4, &boundVert, sizeof(int), &boundT, sizeof(double), &boundU, sizeof(double), &boundV, sizeof(double));
		boundT[nVert] = tBegin;
		boundU[nVert] = u;
		boundV[nVert] = v;
		...
		t = tBegin;
		for (j=0; ; j++) {
			if (nextTinEdge(pm, iSurf, iV_U, t, &t)) break;
	...
	pm->tria = NULL,  pm->nTria = 0,  pm->maxTria = 0;
	...
	for (iSurf=0; iSurf<nSurface; iSurf++) {
		...
		for (i=0; i<pm->nTria; i++) {
			addFace(pm, pm->tria[3*i+0], pm->tria[3*i+1], pm->tria[3*i+2], 0, iLabel);
			if (bCut)
				addFace(pm, pm->tria[3*i+0], pm->tria[3*i+2], pm->tria[3*i+1], 1, bCut);
		}
	}/*for(iSurf<nSurface)*/

	free(boundVert),  free(boundT),  free(boundU),  free(boundV),  nnVert = 0,  boundVert = NULL,  boundT = NULL,  boundU = NULL,  boundV = NULL;
	free(vVert);
	nLine = *pnLine;
	for (iLine=0; iLine<nLine; iLine++) {
	...
	free(surface);
	free(pm->tria),  pm->tria = NULL,  pm->maxTria = 0;
	return;
}
