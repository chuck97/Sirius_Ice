...
static void reallocTree32(surface_mesh *pm, int nnE);
...
static int treewsrec(mesh32 *pm2, surface_mesh *pm, octreenode *c, double *bb, double s, double *x, double r) {
	...
	if (wscheck(pm2, c->e[i], x, r)) {
	    myReallocv(pm->tnVicinityFace+1, &pm->tmaxVicinityFace, 16, 1, (void**)&pm->tvicinityFace, sizeof(PStrucEdge3));
	    pm->tvicinityFace[pm->tnVicinityFace] = pm->edge[pm2->e[c->e[i]].h];
	    pm->tnVicinityFace++;
	}
    }
    k += c->ne;
...
PStrucEdge3 addFace32(surface_mesh *pm, int v1, int v2 ) {
    ...
    if (myReallocv(pm->nEdge+1, &pm->nnEdge, 1024, 1, (void**)&pm->edge, sizeof(PStrucEdge3))) {
	reallocTree32(pm, pm->nnEdge);
    }
    face = myAlloc(sizeof(StrucEdge3));
    face->fail = 0;
