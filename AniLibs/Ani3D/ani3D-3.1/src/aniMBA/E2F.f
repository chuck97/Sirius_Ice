...
C ... number of inverted elements
      ...
      if(flagTM) Then
         nBad = 0
         If(qEs(nE1).LE.0D0) nBad = nBad + 1
         If(qEs(nE2).LE.0D0) nBad = nBad + 1
         If(qEs(nE3).LE.0D0) nBad = nBad + 1

c        If(nBad.GT.0) Goto 1000
      End if
...
C ... updating the list of inverted elements
      If(flagTM) Then
