...
C     collapse of S+S to S requires analysis of faces
      If(ifXnode(ICP1, jSnode) .AND. ifXnode(ICP2, jSnode)) Goto 1000
...
C ... checking for boundary elements
      If(ifXnode(status, ANIForbidBoundaryElements)) Then
         If(ifXnode(ICPs, jInode)) Goto 30
...
C ======================================================================
C Routine checks orientation of {iP1/iP2, iPa, iPb, *} and IPE. 
C
