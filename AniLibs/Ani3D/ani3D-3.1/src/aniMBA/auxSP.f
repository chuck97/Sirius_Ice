...
c           Call delXnode(status, ANITangledMesh)
c           If(iPrint.GE.2) Write(*, 5001) 
         End if
      End if
...
c ... inform the user about requested features  
      If(iPrint.GE.2) Then
         ...
         If(ifXnode(status, ANITangledMesh))            Write(*, 5008) 
...
c           Call addXnode(status, ANIUse2ArmRule)
         End if
 ...
 5008 Format('status.fd: +512  [ANITangledMesh]            [research]')
