...
void DagDrawingTool::draw_DAG( DLIList<TopologyEntity*>& node_list, int up, int down )
{
	int i;
	assert( (up >= 0) && (down >= 0) );
	if( node_list.size() == 0 ) return;
	
	DLIList<TopologyEntity*> relatives[2];
...
{
	DLIList<TopologyEntity*> node_list;
	for( int i = entity_list.size(); i > 0; i-- )
	{
		RefEntity* re_ptr = entity_list.get_and_step();
		TopologyEntity* me_ptr = CAST_TO( re_ptr, TopologyEntity);
...
/*
int DagDrawingTool::name_width( TopologyEntity* node_ptr )
{
  if( node_ptr == NULL )
     return 4;
...
*/
/*
const char* DagDrawingTool::name( TopologyEntity* node_ptr )
{
	...
	TopologyEntity* entity_ptr = node_ptr;
	if( ! entity_ptr )
	{
		strcpy( buffer, "NODE" );
		return buffer;
...
void DagDrawingTool::get_relatives( TopologyEntity* source_ptr,
	DLIList<TopologyEntity*>& result_set, int direction )
{
...
int DagDrawingTool::link_type( TopologyEntity* source_node,
	TopologyEntity* target_node, int direction )
{
...
void DagDrawingTool::printDag(DLIList<TopologyEntity*> &entity_list, int depth)
{
  int i;
...
void DagDrawingTool::printDag( TopologyEntity* ME_ptr, int depth )
{
  assert( ME_ptr != NULL );
...
void DagDrawingTool::print_dag( TopologyEntity* any_dag_node, int depth )
{
...
void DagDrawingTool::print_node( TopologyEntity* node_ptr, int indent, const char* prefix )
{
...
int DagDrawingTool::get_id( TopologyEntity* node_ptr )
{
...
void DagDrawingTool::print_dag( TopologyEntity* node_ptr, int indent, int depth )
{
