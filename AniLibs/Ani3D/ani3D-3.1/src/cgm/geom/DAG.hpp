...
//                 to DAGs and TopologyEntitys. Its current functionalities:
//                   1)Maintain a list of deactivated TopologyEntitys
//		 Note: The "nodes" of the DAG are type TopologyEntity.
//
...
public:
  ...
  void add_deactivated_DAG_node(TopologyEntity* deactivatedDAGNodePtr) ;
    ...
    //I- A pointer to a deactivated DAG node (TopologyEntity)
...
// Methods added by Jason Kraftcheck on September 16, 1996.
  ...
  CubitStatus get_children_at_level( TopologyEntity* parent, int level,
  ...
  CubitStatus get_parents_at_level( TopologyEntity* child, int level,
                                    DLIList<TopologyEntity*>& result_set );
    //R CubitStatus
    //R- CUBIT_SUCCESS/CUBIT_FAILURE
    //I child
  ...
  CubitStatus get_children_at_level( DLIList<TopologyEntity*>& parents, int level,
                                     DLIList<TopologyEntity*>& result_set );
    //R CubitStatus
    //R- CUBIT_SUCCESS/CUBIT_FAILURE
    //I parents
  ...
  CubitStatus get_parents_at_level( DLIList<TopologyEntity*>& children, int level,
                                    DLIList<TopologyEntity*>& result_set );
    //R CubitStatus
    //R- CUBIT_SUCCESS/CUBIT_FAILURE
    //I child
