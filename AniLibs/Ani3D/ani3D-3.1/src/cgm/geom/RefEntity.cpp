...
RefEntity::RefEntity()
{
  ...
  mColor               = CUBIT_DEFAULT_COLOR_INDEX;
  localTolerance       = 0.0;
...
CubitStatus RefEntity::remove_entity_name(CubitString const & name)
{
  ...
  RefEntityName::instance()->remove_refentity_name(this, name);
  AppUtil::instance()->send_event(GeometryEvent(GeometryEvent::ENTITY_NAME_CHANGED, this));
...
DagType RefEntity::dag_type( const std::type_info& type )
{
  if (type == typeid(Body))
    return DagType::body_type();
...
void RefEntity::notify_sub_all_observers(const GeometryEvent::Type& event)
{
...
const std::type_info& RefEntity::get_entity_type_info(const char* entity_type)
{
...
#ifdef CAT
const char* RefEntity::get_ref_class_name(const std::type_info& ref_type)
{
...
{
  mColor = value;
  AppUtil::instance()->send_event(GeometryEvent(GeometryEvent::ENTITY_GEOMETRY_COLOR_CHANGED, this));
}
