...
RefEntityFactory::RefEntityFactory(bool make_lists) 
{
  instance_ = this;
...
RefEntity* RefEntityFactory::get_ref_entity (const std::type_info& type, int id)
{
   // Call the right function, cast it.
...
void RefEntityFactory::maximum_id (const std::type_info& type, int max_id)
{
  if( type == typeid(RefGroup) )
...
void RefEntityFactory::notify_observer(const CubitEvent *observer_event)
{
