...
public :
  ...
  const std::type_info& entity_type_info() const { return typeid(RefFace); }
  ...
  int num_poles();
    //- return the number of singular poles on this surface
...
//========  Change End by DZ of Cat,  on 10/29/98 8:46:59 AM  ========
  ...
  void find_closest_points_trimmed(std::vector<CubitVector> &from_points, 
...
template <> struct DLIListSorter<RefFace*>
{
  bool operator()(RefFace* a, RefFace* b) { return a->id() < b->id(); }
};
