...
public :
  ...
  /*!  Aligns the body that contains my_face to match my_face
    ...
    */
  //! \brief Compute the transformation required to align my_face with target_face with respect to reference_body_ptr.  
  //! Then perform the transformation on all entities in entities_to_move
  CubitStatus align_entities( RefFace *my_face,                          
                          RefFace *target_face,
  ...
  /*!   \return  CubitStatus
    ...
    *    \arg my_face - The surface to align    
    *    \arg target_face - The surface to align to
    *     entities to move - Entities that will be aligned.  Must
          containe body_ptr.  (can include merged ones)
    *    \arg reverse - If true, normals will be pointing opposite one another.
    *    \arg my_center - Returned center of rotation
    *    \arg axis_of_rot - Axis of rotation
    *    \arg target_center - Returned center of target face
    *    \arg angle - Returned angle of rotation    
    *    \arg preview -- If true, just draws the rotated edges
    *   This function will rotate 'entities_to_move' about the center point
    *   of my_face, such that the normal of my_face's center is aligned with 
    *   the nomal at target_face's center and then translates everything
    *   by a vector from my_face's center point to target_face's center point.    
    */


  //! \brief Aligns entities by rotating about them about axis from one point to another
  CubitStatus align_entities(DLIList<RefEntity*> &entities_to_move,
    ...
    /*!  \return  CubitStatus
    *    \return - success or failure
    *    \arg entities - Entities to align
    *    \arg position_to_align - Where the angle of rotation starts
    *    \arg position_to_align_to - Where the angle of rotation ends
    *    \arg axis_origin - Origin of axis_or_rot
    *    \arg axis_of_rot - Axis of rotation
    *    \arg angle_of_rotation - Returned angle of rotation
    *    \arg preview -- If true, just draws the rotated edges
    *   This function will rotate 'entities_to_move' about an axis defined by 
    *   axis_origin and axis_of_rot.  The angle of rotation about the axis 
    *   (right-hand-rule) is the angle between two vectors, v1 and v2, where 
    *   v1 and v2 are vectors perpendicular to the axis, to position_to_align 
    *   and position_to_align_to respectively.
    */


  //! \brief Aligns a the normal of a surface or a line to a plane. 
  CubitStatus align_entities( DLIList<RefEntity*>& reference_entities,
                              DLIList<RefEntity*> &entities_to_move,
    ...
    /*!  \return  CubitStatus
    *    \return - success or failure    
    *    \arg ref_ent_list - The plane you want to align to, defined by 
          either 2 vertices or a surface    
    *     entities to move - Entities that will be aligned (can include merged ones).
    *    \arg align_to_vec - Vector defining the plane to align to
    *    \arg my_center_1 - Returned center of rotation
    *    \arg axis_of_rot - Returned axis of rotation
    *    \arg angle - Returned angle of rotation
    *    \arg preview -- If true, just draws the rotated edges
    *   If ref_ent_list contains a surface, A, this function will rotate 
    *  entities_to_move about A's center point so that A's normal at the center 
    *  point is aligned with vec_to_align_to.
    *   If ref_ent_list contains two vertices, this function will rotate 
    *  entities_to_move about the first vertex so that a vector from vertex 1
    *  to vertex 2 is aligned with vec_to_align_to. 
    */
  ...
  */ 
  ...
  //! \brief Beta function.
  void march_path_to_discover_horizontal(CubitVector &start_pos,
  ...
  void march_path_to_discover_vertical(CubitVector &start_pos,
                 CubitVector &sweep_dir,
                 RefFace *start_face,
                 ...
                 DLIList<CubitVector> &final_points,
                 DLIList<RefFace*> &ending_faces);
  void march_using_planes(CubitVector &point_on_surf,
                          RefFace *surf,
                          CubitVector &march_dir,
                          double step_size,
                          DLIList<CubitVector> &horizontal_points,
                          DLIList<CubitVector> &vertical_points);
  void trace_out_curves(Body *plane_body,
  ...
  */
  ...
  /*!  \return  RefVertex*
    ...
    *  defaults to CUBIT_DEFAULT_COLOR_INDEX
    */
  ...
  RefEdge* make_RefEdge(RefEdge *ref_edge, 
                        bool copy_attribs = true,
                        std::map< RefEntity*, RefEntity* > *old_to_new_map = NULL ) const;
  ...
  */
  ...
  Body* create_parallelogram_surface( RefVertex *v1, 
                                            RefVertex *v2,
                                            RefVertex *v3 );

  Body* create_circle_surface( double radius, CubitVector plane ); 

  Body* create_circle_surface( RefVertex *v1, 
                                     RefVertex *v2,
                                     RefVertex *v3 );
  ...
  Body* create_ellipse_surface( RefVertex *v1, 
                                      RefVertex *v2,
                                      CubitVector center_point ); 

  Body* create_ellipse_surface( double major_radius, 
                                      double minor_radius,
                                      CubitVector plane );
...
private :
  ...
  */
  ...
  void preview_align( CubitVector translation_to_origin,
                      CubitVector origin_to_target,
                      double angle_of_rotation,
                      CubitVector axis_of_rotation,
                      DLIList<RefEntity*> &entities );

  static CubitStatus create_old_to_new_ref_ent_map( 
    TopologyBridge *old_top_level_bridge,
