...
void CubitUndo::retain_mesh_on_result_bodies( DLIList<Body*> &bodies )
{
  if(!mInstance ) return;
  mInstance->p_retain_mesh_on_result_bodies( bodies );
}

void CubitUndo::retain_mesh_on_result_curves( DLIList<RefEdge*> &curves )
{
  if(!mInstance ) return;
  mInstance->p_retain_mesh_on_result_curves( curves );
}

void CubitUndo::retain_mesh_on_result_vertices( DLIList<RefVertex*> &verts )
{
  if(!mInstance ) return;
  mInstance->p_retain_mesh_on_result_vertices( verts );
}
