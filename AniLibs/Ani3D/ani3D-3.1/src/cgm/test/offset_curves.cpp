...
CubitStatus make_Point()
{
  ...
  CubitStatus rsl = gmti->offset_curves(offset_edges, output_edge_list, 1,  v12);
  ...
  rsl = gmti->offset_curves(offset_edges, output_edge_list, 1,  offset_v);
  ...
  rsl = gmti->offset_curves(offset_edges, output_edge_list, 1, v_null);
  assert(rsl == CUBIT_FAILURE);
  ...
  rsl = gmti->offset_curves(offset_edges, output_edge_list, -1,  v12);
  ref_edges.clean_out();
  gti->ref_edges(ref_edges); 
  ref_edges.last();
  offset_edge = ref_edges.get();
  ...
  rsl = gmti->offset_curves(offset_edges, output_edge_list, 1.0, v_null);
  assert(rsl == CUBIT_FAILURE);
  ...
  rsl = gmti->offset_curves(offset_edges, output_edge_list, 2.0, v_null );
  assert(rsl == CUBIT_FAILURE);
