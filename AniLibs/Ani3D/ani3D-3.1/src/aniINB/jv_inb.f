...
c z = J*P(inverse)*v or z = P(inverse)*v is desired (itask = 1,2,-1,-2), 
c then compute P(inverse)*v in rwork1. 
c ------------------------------------------------------------------------
      if (itask .eq. 0) then 
         call dcopy(n, v, 1, rwork1, 1)
      else
        if (itask .lt. 0) then ! preconditioner can be reinitialized
            call dcopy(n, xcur, 1, rwork1, 1)    ! rwork1 on input has xcur
            call prevec(IPREVEC,1,v,rwork1,iW,rW)
        else
            call prevec(IPREVEC,0,v,rwork1,iW,rW)
        end if
      endif
c ------------------------------------------------------------------------
c If only z = P(inverse)*v is desired (itask = 2, -2), then copy rwork1 into 
c z and exit.
c ------------------------------------------------------------------------
      if (itask .eq. iabs(2) ) then 
         call dcopy(n, rwork1, 1, z, 1)
         itrmjv = 0
         go to 900
      endif
c ------------------------------------------------------------------------
c If z = J*v or z = J*P(inverse)*v is desired (itask = 0, 1, -1), then 
